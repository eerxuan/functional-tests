...
from documentdb_tests.framework.assertions import assertSuccess, assertFailureCode, assertSuccessPartial
...
def test_insert_one_document(collection):
    ...
    result = execute_command(collection, {"insert": collection.name, "documents": [{"_id": 0, "a": "A", "b": 30}]})
...
def test_insert_many_documents(collection):
    ...
    result = execute_command(collection, {"insert": collection.name, "documents": [{"_id": 0, "a": "A", "b": 30}, {"_id": 1, "a": "B", "b": 25}, {"_id": 2, "a": "C", "b": 35}]})
...
def test_insert_with_custom_id(collection):
    ...
    result = execute_command(collection, {"insert": collection.name, "documents": [{"_id": "custom_123", "a": "A"}]})
...
def test_insert_duplicate_id_fails(collection):
    ...
    execute_command(collection, {"insert": collection.name, "documents": [{"_id": "dup", "a": "A"}]})
    result = execute_command(collection, {"insert": collection.name, "documents": [{"_id": "dup", "a": "B"}]})
...
def test_insert_nested_document(collection):
    ...
    result = execute_command(collection, {"insert": collection.name, "documents": [{"_id": 0, "a": "A", "b": {"b1": 30, "b2": {"b3": "NYC"}}}]})
...
def test_insert_array_field(collection):
    ...
    result = execute_command(collection, {"insert": collection.name, "documents": [{"_id": 0, "a": "A", "b": ["python", "mongodb"], "c": [95, 87]}]})
