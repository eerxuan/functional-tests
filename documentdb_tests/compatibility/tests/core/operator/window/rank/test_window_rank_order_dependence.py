"""
Tests for $rank order dependence and partition semantics.

$rank is order-dependent: ranks are assigned in sortBy order, so reversing the
sort direction or sorting on a different field produces different ranks for the
same documents. Ranking restarts at 1 in every partition. Unlike
$documentNumber, tied documents share the same rank and the next rank skips
...
def test_rank_ascending_sort(collection):
    """Ascending sort assigns sequential ranks when all values are distinct."""
...
def test_rank_descending_sort(collection):
...
def test_rank_sort_on_different_field(collection):
...
def test_rank_restarts_per_partition(collection):
...
def test_rank_without_partitionBy(collection):
...
def test_rank_single_document_partition(collection):
...
def test_rank_empty_collection(collection):
    """$rank on an empty collection returns no documents without error."""
    result = run_window_operator(
        collection,
        "$rank",
        [],
