...
def validate_test_format(file_path: str) -> list[str]:
    ...
    # Add known helper functions from framework.utils that call execute_command
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and not node.name.startswith('test_'):
            ...
            has_execute = any(
                ...
                n.func.id in ('execute_command', 'execute_admin_command')
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and node.name.startswith('test_'):
            ...
            for n in ast.walk(node):
                if isinstance(n, ast.Assert):
                    errors.append(
                        ...
                        f"Don't use plain 'assert'. Use assertSuccess(), assertFailure(), or assertResult() instead."
                    )
            ...
            call_count = sum(
                ...
                isinstance(n.func, ast.Name) and 
                n.func.id in ('assertSuccess', 'assertFailure', 'assertResult', 'assertFailureCode', 'assertNaN')
            )
            ...
            has_helper_with_execute = any(
                isinstance(n, ast.Call) and 
                isinstance(n.func, ast.Name) and 
                n.func.id in helper_functions_with_execute
                for n in ast.walk(node)
            )
                ...
                if (isinstance(decorator, ast.Call) and 
                    ...
                    for arg in decorator.args:
                        if isinstance(arg, ast.List):
                            for elt in arg.elts:
                                if isinstance(elt, ast.Name) and elt.id in helper_functions_with_execute:
            ...
            if not has_execute_command and not has_helper_with_execute and not has_executor_param:
                errors.append(
                    f"  Function '{node.name}' at line {node.lineno}: "
                    f"Must use execute_command(), execute_admin_command(), or helper functions from framework.utils for MongoDB operations"
                )
